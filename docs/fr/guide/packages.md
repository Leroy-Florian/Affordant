...
Affordant est une petite famille réunie dans un unique monorepo npm-workspaces. Ses paquets partagent un seul contrat d'échange et restent symétriques de part et d'autre : le serveur construit ce que le client consomme.
...
|---|---|---|---|
| [`@affordant/contract`](/fr/reference/contract) | partagé | — | Les types du contrat d'échange. Zéro exécution, zéro dépendance. Tout le reste en dépend. |
...
| [`@affordant/react`](/fr/reference/react) | client | contract, affordant, *react* | Adaptateur React : conditionnez l'UI aux actions proposées et suivez-les avec des hooks. |
